...
import {
  LayoutDashboard, ListChecks, FileText, User, Settings, LogOut, Zap, ChevronRight
} from 'lucide-react';
...
export default function Sidebar() {
  ...
  return (
    <aside className="w-64 min-h-screen flex flex-col bg-gray-900/50 border-r border-white/10 shrink-0">
      {/* Logo */}
      ...
      <nav className="flex-1 p-4 space-y-1">
        {navItems.map(item => {
          ...
          return (
            ...
            >
              <item.icon className={`w-4 h-4 ${isActive ? 'text-indigo-400' : 'text-gray-500 group-hover:text-gray-300'}`} />
              {item.label}
